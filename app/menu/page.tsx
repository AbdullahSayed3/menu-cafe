...
export default function MenuPage() {
  return (
    <div className={`max-w-7xl mx-auto px-4 py-6 bg-background min-h-screen ${cafeFont.className}`}>
      <ThemeToggle />
              ...
              {section.items.map((item) => (
                ...
                >
                  <Image
                    ...
                    width={130}
                    height={130}
                    className="w-32 h-32 object-cover rounded-3xl border border-accent/20 shadow-[0_4px_14px_rgba(0,0,0,0.15)] mb-2 transition-all duration-300"
                  />
                  <CardContent className="text-center p-0 pt-1 w-full space-y-1">
                    ...
                    {item.price && (
                      <p className="text-accent font-extrabold text-sm">{item.price} EGP</p>
                    )}
                    {item.description && (
