...
export default function MenuPage() {
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 bg-[#27252d] min-h-screen">
              ...
              {section.items.map((item) => (
                ...
                >
                  <Image
                    ...
                    width={120}
                    height={120}
  className="w-32 h-32 object-cover rounded-2xl border border-[#cb2663]/10 shadow-[0_4px_12px_rgba(39,37,45,0.5)] mb-2 transition-all duration-300"
                  />

                  <CardContent className="text-center p-0 pt-1 w-full space-y-1">
                    <h3 className="text-sm font-semibold text-[#27252d] leading-snug">
                      {item.name.ar} / {item.name.en}
                    </h3>

                    {item.price && (
                      <p className="text-[#cb2663] font-bold text-sm">{item.price} EGP</p>
                    )}

                    {item.description && (
                      <p className="text-[11px] text-[#6d6f76] leading-tight">
                        {item.description}
                      </p>
                    )}
                  </CardContent>
